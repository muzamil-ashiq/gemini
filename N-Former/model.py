...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # learned per-head modulation to emulate Self-Reinforcing Attention
        # and Synaptic Fatigue inside the fast (flash) attention path
        # alpha: positive -> amplify queries for that head
        # fatigue_coef: positive -> dampen queries proportional to their energy
        self.alpha = nn.Parameter(torch.zeros(self.n_head))
        self.fatigue_coef = nn.Parameter(torch.ones(self.n_head) * 0.05)
    ...
    def forward(self, x):
        ...
        if self.flash:
            ...
            # apply per-head learned modulation to queries to emulate
            # Self-Reinforcing Attention (alpha) and Synaptic Fatigue (fatigue_coef)
            # q: (B, nh, T, hs)
            # compute per-head energy: mean squared magnitude over (B, T, hs)
            with torch.no_grad():
                q_energy = (q * q).mean(dim=(-1, -2))  # (B, nh)
            # average across batch to get stable head-level energy
            q_energy_mean = q_energy.mean(dim=0)  # (nh,)
            # scale factor and damping
            scale = (1.0 + self.alpha).view(1, -1, 1, 1)  # (1, nh, 1, 1)
            damp = (1.0 + (self.fatigue_coef * q_energy_mean).view(1, -1, 1, 1))
            q_mod = q * scale / damp
            y = torch.nn.functional.scaled_dot_product_attention(q_mod, k, v, attn_mask=None, dropout_p=self.dropout if self.training else 0, is_causal=True)
        else:
            ...
            att = att + bonus
            # Synaptic Fatigue
            fatigue = torch.log(torch.exp(att).sum(dim=-1, keepdim=True))
            att = att - fatigue * 0.1
